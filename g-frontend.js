(async () => {
    ...
    const CONFIG = {
		...
		DATABASE_NAME: "consumer-electronics",
		SITE_NAME: "Diagram",
    ...
    const fetchAPI = async (endpoint) => {
        ...
        try {
            ...
            if (response.status === 429) {
                renderRateLimitMessage((await response.json()).error);
                return null;
            }
            if (!response.ok) throw new Error("Server Error");
            const data = await response.json();
            if (data.status === "success") memoryCache.set(endpoint, data);
            return data;
        } catch (e) { return null; }
    };
    ...
    const renderSkeletonHome = () => { /* ... skeleton html ... */ };
    const renderSkeletonDetail = () => { /* ... skeleton html ... */ };
    const renderNoConnection = async () => { /* ... error UI html ... */ };
    ...
    const loadHome = async () => {
        ...
        <dl class="listNews" style="margin-bottom:20px;">
            <small class="text-muted">${formatUTCDate(news.created_at)}</small>
    ...
    const loadDetail = async (slug, type = "news") => {
        ...
        const [resDetail, resBacklinks, resRelated] = await Promise.all([
            ...
            fetchAPI('/api/backlinks').catch(() => null),
		...
		const pubDate = formatUTCDate(news.created_at || news.updated_at);
    ...
    const renderRawXml = async (type) => {
		try {
			...
			const res = await fetch(targetUrl, {
				method: 'GET',
				headers: { 'x-api-key': CONFIG.API_KEY, 'Accept': 'application/xml' }
			});
			...
			document.body.innerHTML = `<div style="padding:50px; font-family:sans-serif; text-align:center;"><h2 style="color:red;">XML Render Error</h2><p>${e.message}</p></div>`;
		}
	};
    
    const lowerPath = pathName.toLowerCase();
	if (pageParam === 'sitemap' || lowerPath.endsWith('sitemap.xml') || lowerPath.endsWith('atom.xml')) {
		await renderRawXml('sitemap'); // Routing otomatis Atom/Sitemap GSC ditarik sitemap.xml
	} 
