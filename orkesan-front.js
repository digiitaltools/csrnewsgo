(async () => {
	...
	const formatUTCDate = (d) => d ? new Date(d).toISOString() : "";
    ...
    const loadHome = async () => {
        ...
        applySeoMeta({
			...
			url: `${CONFIG.DOMAIN}/`,
			type: 'article',
			createdAt: res.data && res.data.length > 0 ? formatUTCDate(res.data[0].created_at) : "" 
		});
    ...
    const loadDetail = async (slug, type = "news") => {
		...
		const pubDate = formatUTCDate(news.created_at);
		const cleanTitle = toTitleCase(news.keyword);
		...
		const schemaGraph = {
			...
			"@graph": [
				...
				},
				{
					...
					"dateModified": pubDate,
					"articleSection": type,
					"author": { "@id": `${CONFIG.DOMAIN}/#person`, "name": "Admin" },
					"publisher": { "@id": `${CONFIG.DOMAIN}/#person` },
        ...
        const detailHtml = `<div class="row">
                ...
                <aside class="col-sm-4">
                    <div id="ads-320x50" style="width:320px; height:50px; margin-bottom:30px; background:#f0f0f0; display:flex; align-items:center; justify-content:center; color:#999; border-radius:8px; margin-left:auto; margin-right:auto;">ADVERTISEMENT</div>
	...
	if (pageParam === 'sitemap' || lowerPath.endsWith('sitemap.xml') || lowerPath.endsWith('atom.xml')) {
		...
		// Perbaikan: Pastikan iklan banner atas (728x90) dan sidebar (320x50) dipanggil dengan sinkron
