(async () => {
						...
						<div class="col-xs-4 col-md-6 col-lg-8">
							<div class="logo-brand text-center">
								<a href="/" class="mh-auto">
									<img id="main-logo" src="https://cdn.statically.io/gh/digiitaltools/csrnewsgo@main/n1_ipotnews.png" class="img-responsive hidden-xs hidden-sm mh-auto">
								</a>

								<a href="/" class="mh-auto">
									<img id="main-logo-mobile" src="https://cdn.statically.io/gh/digiitaltools/csrnewsgo@main/n1_ipotnews_w.png" class="img-responsive visible-xs visible-sm mh-auto">
								</a>
							</div>
						</div>
    ...
    const loadHome = async () => {
		try {
			...
			if (result.status === "success") {
				if (result.config && result.config.type === 'path') {
					isTldMode = true;
				} else {
					isTldMode = false;
				}
    ...
    const loadDetail = async (slug) => {
		try {
			...
			if (d.config && d.config.type) {
				isTldMode = (d.config.type === 'path');
			} else {
