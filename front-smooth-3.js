(async () => {
    ...
    const loadDetail = async (slug, type = "news") => {
		...
		const detailDescription = limitText(
			...
			(Array.isArray(contentData) ? contentData.join(' ') : contentData) || detailTitleBase, 160
		);
		const detailKeywords = makeKeywordString([news.keyword, news.title, news.keywords, news.tags, news.category, slug, CONFIG.DEFAULT_KEYWORDS]);
