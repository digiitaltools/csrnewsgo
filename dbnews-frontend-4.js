(async () => {
    ...
    const renderRawXml = async (type) => {
		try {
			...
			const targetUrl = `${CONFIG.API_URL}/api/${type}?key=${CONFIG.API_KEY}&domain=${encodeURIComponent(CONFIG.DOMAIN)}&db=${CONFIG.DATABASE_NAME}&format=${urlFormat}`;
			...
			document.body.innerHTML = `
				<div style="padding:50px; font-family:sans-serif; text-align:center;">
					<h2 style="color:red;">XML Error</h2>
					<p>${e.message}</p>
